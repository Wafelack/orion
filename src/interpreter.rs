...
mod test {
    ...
    #[test]
    fn ackermann() -> Result<()> {
        let tokens = Lexer::new(r#"(def ackermann (λ (m n)
    (match (, m n)
        ((, 0 _) (+ n 1))
        ((, _ 0) (ackermann (- m 1) 1))
        (_ (ackermann (- m 1) (ackermann m (- n 1)))))))"#).proc_tokens()?;

        let ast = Parser::new(tokens).parse()?;
        let mut interpreter = Interpreter::new(ast);
        interpreter.interpret(false)?;

        let tokens = Lexer::new("(ackermann 3 3)").proc_tokens()?;
        let ast = Parser::new(tokens).parse()?;
        ...
        times.sort();
        println!("Total: {}ms ; Average: {}ms ; Median: {}ms ; Amplitude: {}ms", total, total / times.len() as u128, times[times.len() / 2], times[times.len() - 1] - times[0]);
